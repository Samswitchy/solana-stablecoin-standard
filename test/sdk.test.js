...
test("create requires required fields", async () => {
  await assert.rejects(
    () => SolanaStablecoin.create({}, { preset: Presets.SSS_1 }),
    /Missing required config field/
  );
});

test("mint + transfer + burn updates balances and supply", async () => {
  const s = await SolanaStablecoin.create({}, {
...
test("pause prevents minting", async () => {
  ...
  await assert.rejects(
    () => s.mint({ recipient: "alice", amount: 1, minter: "m1" }),
    /paused/
  );
});
...
test("quota enforcement blocks over-minting", async () => {
  ...
  await assert.rejects(
    () => s.mint({ recipient: "alice", amount: 50, minter: "m1" }),
    /quota exceeded/
  );
});
